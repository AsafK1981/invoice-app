...
vi.mock("@/lib/supabase", () => ({ supabase: { from: () => {
  ...
  const q: any = {
    ...
    then(resolve: (v: unknown) => void) { return Promise.resolve(resolve({ data: kind === "read" && state.existing ? { id: "existing" } : null, error: kind !== "read" && state.error ? new Error("write failed") : null })); },
  }; return q;
} } }));
...
describe.each(cases)("%s import store", (_, save) => {
