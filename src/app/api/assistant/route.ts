...
אתה עוזר למשתמש למצוא מסמכים, להבין את הנתונים הפיננסיים שלו, ולהכין טיוטות מסמכים.

כללי עבודה:
- ענה תמיד בעברית, קצר וענייני. בלי הקדמות ובלי "בשמחה!".
- אתה רואה אך ורק את הנתונים של המשתמש המחובר. אין לך גישה לנתונים של אף אחד אחר.
- כשחסר לך מידע כדי לבצע פעולה - שאל שאלה אחת ממוקדת. אל תנחש.
  לדוגמה: אם המשתמש אומר "תוציא קבלה לדני" ויש שני לקוחות בשם דני, שאל איזה מהם.
- אל תמציא נתונים. אם החיפוש לא החזיר תוצאות, אמור זאת במפורש.
- סכומים בשקלים, תאריכים בפורמט DD/MM/YYYY בתשובות למשתמש.

לגבי יצירת מסמכים:
- אתה לא יוצר מסמכים. אתה מכין טיוטה בעזרת prepare_document_draft, והמשתמש
  פותח אותה בעורך, בודק ומאשר בעצמו. תמיד הסבר שזו טיוטה שממתינה לאישורו.
- לפני הכנת טיוטה ודא שיש לך: סוג המסמך, הלקוח, ומה השורות (תיאור + סכום).
...
const TOOLS: Anthropic.Tool[] = [
  {
    ...
    description:
      "מחפש מסמכים של המשתמש (חשבוניות, קבלות, הצעות מחיר, זיכויים). " +
      "השתמש בכלי הזה בכל פעם שהמשתמש מחפש מסמך, שואל מה שלח למישהו, או צריך רשימה. " +
      `מחזיר עד ${SEARCH_LIMIT} מסמכים, מהחדש לישן. כל הפרמטרים אופציונליים - ` +
      "בלי פרמטרים מחזיר את המסמכים האחרונים.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "מחזיר סיכום הכנסות לתקופה: סך ההכנסה, מספר המסמכים, ופילוח לפי לקוח. " +
      'סופר רק מסמכים שנחשבים הכנסה בפועל (קבלה / חשבונית מס / חשבונית מס-קבלה ששולמו, ' +
      "בניכוי זיכויים, בלי כפילויות של מסמכים שהומרו). " +
      "השתמש בזה לשאלות כמו 'כמה הכנסתי החודש' או 'כמה הכנסתי השנה'.",
    input_schema: {
      type: "object",
      properties: {
        dateFrom: { type: "string", description: "תאריך התחלה, YYYY-MM-DD" },
  ...
  },
  {
    name: "list_clients",
    description:
      "מחזיר את רשימת הלקוחות של המשתמש. השתמש בזה כדי לזהות לקוח לפי שם חלקי " +
      "לפני הכנת טיוטה, או כשצריך לוודא איזה לקוח המשתמש התכוון אליו.",
    input_schema: {
      type: "object",
