...
// Shared with the recurring-pattern cron, the other producer of proposals.
import {
  MAX_DETAILS,
  MAX_SOURCE_LABEL,
  PERIOD_RE,
  ...
  if (issuedMatch) {
    ...
    const { error: resolveErr } = existing
      ...
      : await supabase.from("invoice_proposals").insert({
          ...
          source,
          source_label: clean(body.sourceLabel, MAX_SOURCE_LABEL) || source,
          period,
