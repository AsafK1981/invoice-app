-- Israel Invoice v2 allocation requires the CUSTOMER's עוסק number
-- (customer_vat_number, mandatory, must be >= 10). We capture it in the editor
-- (ad-hoc "ח.פ / ת.ז" field, or a saved client's tax_id) but never persisted
-- it on the document — so ad-hoc invoices had no way to supply it at allocation
-- time. Add documents.client_tax_id and thread it through create_document_atomic.

ALTER TABLE documents ADD COLUMN IF NOT EXISTS client_tax_id text;

-- Recreate the create-document RPC with the new p_client_tax_id parameter.
-- Drop the old exact signature first so we don't leave an overload that makes
...
DROP FUNCTION IF EXISTS public.create_document_atomic(
  ...
  text, text, jsonb, text, numeric, numeric, numeric, numeric, boolean
);
